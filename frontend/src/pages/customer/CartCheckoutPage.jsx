...
export default function CartCheckoutPage() {
  ...
  useEffect(() => {
    if (requiresPrescription && accessToken) {
      setLoadingRx(true);
      api
        .getApprovedPrescriptions(accessToken)
  ...
  const handlePlaceOrder = async () => {
    ...
    if (requiresPrescription && !selectedRxId) {
      addToast(
        "Please select an approved prescription for prescription-only medicines.",
        "error",
      );
      return;
    }
    try {
      ...
      if (paymentMethod === "imepay" && order) {
        try {
          const payRes = await api.initiateImepay(
            { orderId: order.id },
            accessToken,
          );
          ...
          addToast(
            "IME Pay initiation failed. You can pay later from order details.",
            "error",
          );
        }
      }
  ...
  if (orderPlaced) {
    return (
      <div className="max-w-md mx-auto text-center py-12 page-enter">
        ...
        <h2 className="font-fraunces text-2xl font-semibold text-charcoal">
          Order confirmed
        </h2>
        <p className="text-charcoal/60 mt-2">
          Order #{orderPlaced.orderNumber} — Rs. {orderPlaced.grandTotal}
        </p>
        <p className="text-charcoal/60 text-sm mt-1">
          Thank you for your order. We&apos;ll deliver soon.
        </p>
        <Link
          to="/customer/orders"
          className="mt-6 inline-block rounded-lg bg-primary px-6 py-2.5 font-medium text-white hover:bg-primary-dark"
        >
          View My Orders
        </Link>
      </div>
        ...
        <>
          <div className="rounded-xl border border-charcoal/10 bg-white overflow-hidden">
              ...
              {items.map((item) => (
                ...
                >
                  <div className="h-12 w-12 shrink-0 overflow-hidden rounded-lg bg-charcoal/5">
                    ...
                    <div
                      className={`h-full w-full items-center justify-center text-2xl ${item.imageUrl ? "hidden" : "flex"}`}
                    >
                      💊
                    </div>
                  </div>
                  ...
                  />
                  <p className="font-fraunces font-semibold text-charcoal w-20 text-right">
                    Rs. {item.lineTotal}
                  </p>
                  <button
                    type="button"
            ...
            <div className="border-t border-charcoal/10 p-4 space-y-1 text-right">
              ...
              <p className="font-fraunces text-lg font-semibold text-charcoal">
                Total: Rs. {grandTotal}
              </p>
              {subtotal < freeDeliveryThreshold && (
                <p className="text-xs text-charcoal/50">
                  Add Rs. {freeDeliveryThreshold - subtotal} more for free
                  delivery
                </p>
              )}
            </div>
          ...
          {requiresPrescription && (
            <div className="rounded-xl border border-amber-300 bg-amber-50 p-5 space-y-3">
              ...
              <p className="text-sm text-charcoal/70">
                Your cart contains prescription-only medicines. Select an
                approved prescription to continue.
              </p>
              {loadingRx ? (
                ...
                <div className="space-y-2">
                  <p className="text-sm text-soft-red font-medium">
                ...
                >
                  ...
                  {approvedRx.map((rx) => (
                    <option key={rx.id} value={rx.id}>
                      Approved on{" "}
                      {new Date(rx.updated_at).toLocaleDateString("en-NP", {
                        year: "numeric",
                        month: "short",
                        day: "numeric",
                      })}
        ...
        <>
          ...
          <div className="flex justify-between">
            <button
              type="button"
              onClick={() => setStep(1)}
              className="rounded-lg border border-charcoal/20 px-4 py-2.5 font-medium text-charcoal"
            >
              Back
            </button>
            <button
                ...
                <label
                  ...
                  className={`flex items-center gap-4 rounded-lg border p-4 cursor-pointer transition-colors ${
                    paymentMethod === pm.id
                      ...
                      : "border-charcoal/20 hover:bg-charcoal/5"
                  }`}
                >
                  <input
                    type="radio"
